...
public function action_closeRegistrations() {
    ...
    public function action_eventListAjax() {
        ...
        if (!empty($filters['name']))        $conditions["name[~]"] = $filters['name'];
        if (!empty($filters['description'])) $conditions["description[~]"] = $filters['description'];
        if (!empty($filters['date_from']))   $conditions["date[>=]"] = $filters['date_from'];
        if (!empty($filters['date_to']))     $conditions["date[<=]"] = $filters['date_to'];
        if (!empty($filters['time_from']))   $conditions["time[>=]"] = $filters['time_from'];
        if (!empty($filters['time_to']))     $conditions["time[<=]"] = $filters['time_to'];
        if (!empty($filters['status']))      $conditions["is_verified"] = $filters['status'] == 'verified' ? 1 : 0;
        if (!empty($filters['type']))        $conditions["type_id"] = $filters['type'];
        if (!empty($filters['location']))    $conditions["location[~]"] = $filters['location'];
