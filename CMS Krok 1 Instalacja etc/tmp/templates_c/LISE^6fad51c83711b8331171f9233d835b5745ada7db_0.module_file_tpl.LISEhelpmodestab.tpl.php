...
/* Smarty version 4.2.1, created on 2025-05-02 18:36:52
  from 'module_file_tpl:LISE;help_modes_tab.tpl' */

/* @var Smarty_Internal_Template $_smarty_tpl */
if ($_smarty_tpl->_decodeProperties($_smarty_tpl, array (
  'version' => '4.2.1',
  'unifunc' => 'content_6814f4a4b42ea6_22645924',
  'has_nocache_code' => false,
  'file_dependency' => 
  array (
    '6fad51c83711b8331171f9233d835b5745ada7db' => 
    array (
      0 => 'module_file_tpl:LISE;help_modes_tab.tpl',
      1 => 1746203811,
      2 => 'module_file_tpl',
    ),
  ),
...
function content_6814f4a4b42ea6_22645924 (Smarty_Internal_Template $_smarty_tpl) {
...
<div class="m_top_25"></div>
<p>Let's say, as an example, that you have created a LISE instance called <b>Global</b> in <b><i>global mode</i></b>. LISE will, as usual, create a module instance that will be called <b>LISEGlobal</b>. However it will not register the module in Smarty as usual: <b>{LISEGlobal}</b> won't have the usual parameters you would expect and, particularly, won't have the expected frontend actions. While the tag is still valid, it will have different functions than what you'd expect from the usual LISE mode.</p>

<div class="warning">
  <p><b>LISEGlobal</b>, {LISEGlobal} and {$LISEGlobal} are used here as examples. The actual names will depend on the name you'll give to the modules on creation, and will follow the tipical template: <b>LISE<i>&lt;InstanceName&gt;</i></b></p>
</div>

<p>In Gobal mode the aliases of the created custom fields will be available using the following calls:</p>
<pre><code>{* tag as static class with a get method *}
{LISEGlobal::get('&lt;field_alias&gt;')}

{* variable tag as either an object or an array *}
{$LISEGlobal.&lt;field_alias&gt;}
{$LISEGlobal['&lt;field_alias&gt;']}
{$LISEGlobal->&lt;field_alias&gt;}
</code></pre>
...
<pre><code>{* note the use of the LISEGlobal tag as static class with a get method *}
{LISEGlobal::get('myalias')}

{* otherwise use the $LISEGlobal variable tag as either an object or an array as below *}
{$LISEGlobal.myalias}
{$LISEGlobal['myalias']}
{$LISEGlobal->myalias}
</code></pre>
...
<div class="m_top_25"></div>
<p>Let's say, as an example, that you have created a LISE instance called <b>Local</b> in <b><i>local mode</i></b>. LISE will, as usual, create a module instance that will be called <b>LISELocal</b>. However it will not register the module in Smarty as usual: <b>{LISELocal}</b> won't have the usual parameters you would expect and, particularly, won't have the expected frontend actions. While the tag is still valid, it will have different functions than what you'd expect from the usual LISE mode.</p>

<div class="warning">
  <p><b>LISELocal</b>, {LISELocal} and {$LISELocal} are used here as examples. The actual names will depend on the name you'll give to the modules on creation, and will follow the tipical template: <b>LISE<i>&lt;InstanceName&gt;</i></b></p>
</div>

<p>In local mode the aliases of the created custom fields will be available using the following calls:</p>
<pre><code>{* tag as static class with a get method *}
{LISELocal::get('&lt;field_alias&gt;')}

{* variable tag as either an object or an array *}
{$LISELocal.&lt;field_alias&gt;}
{$LISELocal['&lt;field_alias&gt;']}
{$LISELocal->&lt;field_alias&gt;}
</code></pre>
...
<pre><code>{* note the use of the LISELocal tag as static class with a get method *}
{LISELocal::get('myalias')}
